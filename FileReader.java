                ...
                try {
                    ...
                    switch (command) {
                        ...
                        case "ADD_TASK2":
                            ...
                            break;
                        case "LIST_ALL_TASKS":
                            String taskFilter = parts.length > 1 ? parts[1] : "";
                            taskManagement.listAllTasks(taskFilter);
                            break;
                        case "TASK_DONE":
                            if (parts.length > 1) {
                        ...
                        case "LIST_ALL_WISHES":
                            String wishFilter = parts.length > 1 ? parts[1] : "";
                            wishManagement.ListAllWishes(wishFilter);
                            break;
                        ...
                        case "WISH_CHECKED":
                            if (parts.length > 1) {
                                if (child.getWishList().isEmpty()) {
                                    ...
                                    String status = parts.length > 2 ? parts[2] : Wish.COMMAND_APPROVED;
                                    int level = parts.length > 3 ? Integer.parseInt(parts[3]) : 2;
                                    wishManagement.wishChecked(wishId, status, level, child);
                                }
                            } else if (child.getWishList().isEmpty()) {
                                System.out.println("No wishes available to check");
                            } else {
                                Wish firstWish = child.getWishList().get(0);
                                wishManagement.wishChecked(firstWish.getId(), Wish.COMMAND_APPROVED, 2, child);
                            }
                            break;
                        case "PRINT_BUDGET":
                            child.printBudget();
        ...
        try (Scanner scanner = new Scanner(new File("Wishes.txt"))) {
            while (scanner.hasNextLine()) {
                ...
                if (line.startsWith("ADD_WISH1")) {
                    ...
                    ProductWish wish = new ProductWish(wishId, title, description);
                ...
                if (line.startsWith("ADD_WISH2")) {
                    try {
                        ...
                        if (lastParts.length < 4) {
                            System.out.println("Warning: Incomplete data for ActivityWish ID: " + wishId);
                            continue;
                        }
                        ...
                        ActivityWish wish = new ActivityWish(wishId, title, description, 
                                LocalDateTime.of(startDate, startTime), 
                        ...
                        System.out.println("Error parsing date/time in ADD_WISH2: " + e.getMessage());
                    } catch (Exception e) {
